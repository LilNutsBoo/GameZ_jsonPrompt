...
• 《GameZ规则书v3.0.docx》：作为本轮规则基础，ChatGPT 必须主动解析并严格遵守其中全部行为规范与生成约束。
...
该风格持续有效，直至用户主动声明更换或终止。
        ...
        with st.expander("🗣️ 每轮剧情前 · 剧情准备（点击展开）", expanded=False):
        ...
        with st.expander("🔧 Canvas 备忘录", expanded=False):
            st.code("""
        📌 启用对话内的 Canvas，并搭建如下结构以便用户手动记录，禁止chatgpt编辑本对话中的canvas
